...
// - 일반적으로 256 byte 이내로 크기 제한이 있는 것으로 알려져 있으나,
//   이 용량은 브라우져 또는 장비에 따라 다를수 있음
// - 결론은 이미지 등 무거운 자료의 전송은 대부분 불가능하므로 이때는 post로 해야함  
...
public class Ex03_GetPost extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// ** Get Test
		// => 쿼리스트링으로 Test
		//	  https://localhost:8080/Web01/getpost?id=banana&name=홍길동&age=22
		
		// ** 요청분석
	    // => 한글처리
	    //   - getParameter 전에 해야함
	    //   - Tomcat(WAS) 은 Get 방식요청에서는 "UTF-8"을 default로 적용
	    ...
	    //   - 해당하는 Parameter (id)가 없으면 null을 return \
