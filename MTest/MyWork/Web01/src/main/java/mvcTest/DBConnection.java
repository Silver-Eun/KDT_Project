...
// ** DB 연결
// => Connection 객체가 DB 연결 및 연결정보를 관리함
// 즉, Connection 객체를 생성해야함

// ** Connection 객체 생성
// => 일반적인 생성문 
//    Connection cn = new Connection_구현클래스() -> XXX
// => DB 연결정보를 이용해서 생성 후 그 생성값을 전달받음   
...
//    사용할 애플리케이션에 대해 사용 가능한 JDBC(Java Database Connectivity) 드라이버 세트를 관리함
...
public class DBConnection {
	public static Connection getConnection() {
		...
		try {
			...
			// => Connector mysql-connector-j-8.1.0.jar내에서 "com.mysql.cj.jdbc.Driver"를 찾음
            //    그러므로 실행 전에 프로젝트에 ~.jar 연동
