...
interface Inter1 {
	float PI = 31.14159f; // public static final 생략
...
public class Ex04_MultiTest {
	...
	public static void main(String[] args) {
		...
		Inter3 in3 = new MultiInter(); // 모두 메서드 접근 가능, MultiInter() / MultiExIm() 교체 가능
		System.out.printf("main in3 => getA = %d, getB = %d, getC = %f\n",
				in3.getA(), in3.getB(), in3.getC());
		
		// 실습 4) instanceof
		// => 비교 대상이 인터페이스인 경우에는 컴파일 오류 없고, 런타임시에 비교 결과(적합성 여부) 알려줌
	    //    그러므로 적합성의 확인이 필요한 경우 사용해야함
		if(m1 instanceof Inter1) System.out.println("m1은 Inter1입니다.");
		else System.out.println("m1은 Inter1이 아닙니다.");
		if(m1 instanceof Inter3) System.out.println("m1은 Inter3입니다.");
		else System.out.println("m1은 Inter3이 아닙니다.");

		if(in1 instanceof Inter3) System.out.println("in1은 Inter3입니다.");
		else System.out.println("in1은 Inter3이 아닙니다.");
		if(in2 instanceof Inter3) System.out.println("in2은 Inter3입니다.");
		else System.out.println("in2은 Inter3이 아닙니다.");
		if(in3 instanceof Inter3) System.out.println("in3은 Inter3입니다.");
		else System.out.println("in3은 Inter3이 아닙니다.");
		
		if(in3 instanceof Inter1) System.out.println("in3은 Inter1입니다.");
		else System.out.println("in3은 Inter1이 아닙니다.");
		
		// ** 비교 Test
		// => interface와 무관한 Add 인스턴스 비교
		Add add = new Add();
		if(add instanceof Inter3) System.out.println("add는 Inter3입니다.");
		else System.out.println("add는 Inter3이 아닙니다.");
		
		// Inter3 in33 = new Add(); -> 그러므로 성립 안됨
	}
