...
// => project, class명은 대문자로 시작.
// => 상수명은 전체대문자로 작성. 
// => 그외에는 모두 소문자로 시작(합성 : setName) 
// => 한글: 허용은 되지만 권장하지 않음
// => 연산자(+,-,*,/,%)는 식별자에 포함할 수 없음
// => 한번 선언된 변수명은 재선언 할 수 없음 
//	( 동일한 identifier를 중복 선언할 수 없음 )
...
public class Ex02_Variable01 {
	...
	public static void main(String[] args) {
		...
		// ** 변수
		// 1) 적합성
		String 이름 = "김자바"; // 가능하지만 비추
		String irum = "김자바"; // 가능하지만 비추
		String Name; // name과 Name은 다르며 변수명의 첫글자 대문자는 바람직하지 않음
		
		// 2) Type
		// 2.1) 정수형
		// => byte(8_1), short(16_2), int(32_4byte), long(64_8byte)
		byte b = 10; // 선언과 동시에 초기화
		short s = 100; // 선언과 동시에 초기화
		int i = 12345; // 선언과 동시에 초기화
	    // ** 정수사용 주의사항
		...
		System.out.println("** (char + char) => " + (aa + bb + cc)); // 우선 순위 : int 연산
		System.out.println("** char + char => " + aa + bb + cc);
		System.out.println("** String + char => " + (aa + bb + cc + ss)); // 순서 적용 : int 연산 후 문자열 연산
		// char를 int로 출력
		System.out.println("** (int)char => " + (int)aa + (int)bb + (int)cc); // int 값, (int)ss : xxxx
		System.out.printf("** A = %d, B =%d, C = %d\n", (int)aa, (int)bb, (int)cc); // 우선 순위 : int 연산
		// => 한글적용
		char k = '가', n = '나';
