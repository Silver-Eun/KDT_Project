...
// * String
// => 불변(immutable) 속성을 가짐. 
//    그러므로 문자열의 연산(입력, 수정, 삭제 등)이 빈번한 경우 힙(Heap)메모리에 
//    많은 가비지(Garbage)가 생성되므로 비효율적.
// => equals 메서드를 오버라이딩해서 문자열의 내용을 비교할 수 있도록 했음.
...
//    그러므로 문자열의 연산(입력, 수정, 삭제 등)이 빈번한 경우 String보다 적합.
...
// => toString은 오버라이딩 되어 있어서 담고있는 문자열을 String으로 반환 한다

// ** StringBuffer클래스 특징
// => 문자열처리 속도 높고, 중간에 변경 가능함
