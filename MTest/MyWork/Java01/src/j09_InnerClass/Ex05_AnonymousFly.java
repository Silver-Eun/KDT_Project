...
class FlyerImpl implements Flyer {
	@Override
	public void fly() {
		System.out.println("1. 일반적인 방법 => 구현클래스 작성 : 재사용가능");
	}
}
...
public class Ex05_AnonymousFly {
	...
	public static void main(String[] args) {
		...
		/*
		 * Flyer f2 = class { ~~~ }; Flyer f2 = new class { ~~~ }; -> 이러한 개념을 문법적으로는
		 * 아래처럼 정의 Flyer f2 = new Flyer(); -> 인터페이스로는 생성불가능, 문법적으로 오류, 인터페이스의 생성자는 허용안됨
		 * Flyer f2 = new Flyer() { ~~~ }; -> 익명 클래스 정의 (허용됨)
		 */
		Flyer f2 = new Flyer() {
			@Override
			public void fly() {
				System.out.println("2. 익명(Anonymous) 클래스 : 오버라이딩 메소드 직접 작성");
			}
			
			// => 오버라이딩 외의 메소드 추가 : 오류는 없으나, 조상의 인스턴스로는 실행불가능하므로 의미없음
