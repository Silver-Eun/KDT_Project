...
// => instance 변수는 객체 또는 인스턴스 수준에서 지역 변수로 동작함
// => 각 인스턴스가 고유한 값을 가지므로, 한 인스턴스에서 변경해도 다른 인스턴스에 영향 주지 않음

// ** 호출규칙
// => static 메소드 : static만 인스턴스없이 호출 가능
// => instance 메소드 : static, instance(non_static) 모두 인스턴스없이 호출 가능
...
public class Ex05_static {
	...
	public static int add(int i, int j, Ex05_static ex00) {
		...
		total += (i + j); // static 메소드에서 STATIC은 인스턴스없이 접근 가능

		// static 메소드에서 인스턴스 추가
		// => static 메소드에서 STATIC은 인스턴스없이 접근 가능
		ex00.itotal += (i + j);
	...
	// => instance
	// => static, instance(non_static) 모두 인스턴스없이 호출 가능
	...
	public static void main(String[] args) {
		...
		// 1.3) staticAll 호출
		System.out.println("1.3) staticAll 호출");
		staticAll(30, 3, ex05);

		// 1.4) instanceAll 호출
		System.out.println("1.4) instanceAll 호출");
		ex05.instanceAll(40, 4, ex05);
		
		// 2) ** instance 추가
		Ex05_static ex55 = new Ex05_static();
		System.out.println("\n2) ** instance 추가");
		
		// 2.1) static call
		System.out.println("add2 => " + add(10, 3, ex55));
		System.out.println("min2 => " + min(10, 3, ex55));
		
		// 2.2)instance(non_static) call
		System.out.println("mul2 => " + ex55.mul(10, 3));
		System.out.println("div2 => " + ex55.div(12, 3));
		
		System.out.println("main2 total => " + total);
		System.out.println("main2 itotal => " + ex55.itotal);
