...
public class Ex01_Basic {
	...
	public static void main(String[] args) {
		...
		// result = x / 0;
		// -> 컴파일 오류는 없음
		// => Exception 발생 행에서 해당하는 Exception 객체를 생성해
		//    메시지를 출력하고 비정상 종료(java.lang.ArithmeticException: / by zero)
		...
		try {
			 ...
			 // 배열크기 음수지정
			 names = new String[5];
			 System.out.println("result4 => " + names.length);
			 s = null;
			 System.out.println("NullPointerException Test => " + s.length());
		} catch (ArithmeticException e) {
			System.out.println("ArtithmeticException => " + e.toString());			
			...
			// Exception에 다형성 적용
			// => Exception e = new 후손()
			// => 즉, Exception의 모든 후손 Exception들이 이 catch 블럭으로 전달됨
			//    ArtithmeticException, NumberFormatException....
			// => 그러므로 각 Exception별로 처리하는것도 가능
			//    이때는 조상 Exception 객체는 제일 마지막에 정의해야함
