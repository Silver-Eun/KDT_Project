...
function idOK() {
	...
	// => 현재 창의 input Tag의 id 값을 부모창의 id값으로 전달
	...
	// 3)
	// => readonly 속성 사용시 주의
	//    Tag의 속성은 readonly로 정의되어 있지만, 
	//    DOM의 node 객체에서는 readOnl 로 정의되어있으므로
	//    JS 코딩시에는 readOnly로 사용해야함
	// opener.document.getElementById('id').readonly = 'readonly'; // XXXX
		...
		<div id="msgBlock">
			...
			<c:if test="${idUse == 'F'}">
				...
				<!-- 부모창(mJoinForm, opener)에 남아있는 사용자가 입력한 id는 지워주고,  
				    현재(this)창 에서는 id에 focus를 주고 재입력 유도 -> script 필요
				-->
			</c:if>
			<script>
