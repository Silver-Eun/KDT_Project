...
// => Around 메서드 1개로 구현
//    Before, After_returning, After_throwing, After를
//    1개의 메서드에서 try~catch~finally 를 이용해서 처리
...
public class MyAspect {
	...
	public void myAround(ProceedingJoinPoint joinpoint) {
		...
		try {
			...
			// => 발생된 예외를 Throwable로 처리(처리&종료) 했으므로 main까지 전달되지않음 (확인 후 Test)
			// => main으로 전달하려면 예외발생시켜주면 됨
