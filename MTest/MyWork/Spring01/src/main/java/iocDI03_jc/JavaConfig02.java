...
public class JavaConfig02 {
	...
	@Bean
	// => 컨테이너에 의해 메소드 실행되고 인스턴스가 컨테이너로 return 됨
	...
	// => @Bean는 해당 인스턴스를 컨테이너로 전달해야할 필요가 있는 경우 사용
	//   있을 때와 없을 때 아래 AiTVs에 sp 인스턴스가 전달되는지 비교해본다. 
	...
	// => AiTVs xml로 생성 : JC 파일의 sp 인스턴스가 전달되는지 확인
