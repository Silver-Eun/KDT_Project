...
// *** JC 와 @ 병행사용
// *** JC , @, xml 병행사용
// => JC내에서 xml로 생성된 객체를 직접 사용하는것은 허용되지 않음
// 단, User 클래스에서는 사용가능

// ** 실습
// => SsTVsi , Speaker는 xml로 생성
// => LgTVsi, AiTVsi는 JC의 @Bean 으로 생성
...
public class JavaConfig03 {
	...
	// 2) IOC/DI: 생성자 주입
	// => SpeakerB를 xml에서 생성한 후 전딜되는지 확인 : 불가능
	// => SpeakerB를 @으로 생성한 후 전딜되는지 확인 : 불가능
	// => 결론 : JC내에서 생성자 주입하려면 JC내에서 생성해야됨
	@Bean
	public TV lgtv() { return new LgTVsi(spb(), "Orange", 3344000); }
	@Bean
	public Speakeri spb() { return new SpeakerB(); }
	// 3) IOC/DI: JC에서 xml 병행 사용
	// => SpeakerA, xml 생성 후 @Autowired로 주입받음
	// => 1) AiTVsi : JC 생성
	@Bean
