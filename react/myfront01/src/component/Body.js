...
function Body() {
  ...
  // => input Tag에서 ref={textRef} 명령으로 textRef가 DOM 입력폼에 접근하도록 설정
  //    그러면 textRef를 이용하여 입력폼을 직접 조작할 수 있음.
  ...
  const onClickBtn = () => {
    ...
    // test2) 포커스 적용
    // => 입력된 text가 3글자 미만이면 포커스가 머문 상태로 입력 기다림
    if (text.length < 3) {
  ...
  return (
    <div className="body">
      <h1>** Body: useRef(Reference) Test</h1>
