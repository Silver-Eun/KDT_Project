...
function startMethod() {
  ...
  xhrObj.open("Get", "../html/230719_01_Promise.html", "true");
  // d) 요청전송
  xhrObj.send(null);
} // startMethod
...
function startMethodG() {
  ...
  let url = "../html/230719_04_parameterResult.html?" + getParameterValues();
  // 230719_04_parameterResult.html?id=banana&password=12345!&name=바나나
  xhrObj.open("Get", url, "true");
  // => 요청전송
  xhrObj.send(null);
} //startMethodG

// => Post : DataValue를 body 영역에 담아 전송
function startMethodP() {
  createXHR();
  xhrObj.onreadystatechange = resultF;
  // => 요청설정
  let url = "../html/230719_04_parameterResult.html";
  xhrObj.open("Post", url, "true");
  // => 요청전송 : parameter를 매개변수로 전달 , request Header 값 처리
