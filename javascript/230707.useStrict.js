...
/*
엄격모드 활성화.  이와 같이 전역적으로 선두에 선언되어야
...
단, 엄격모드 (strict mode) 적용시에는 ReferenceError(n1 is not defined) 발생.
*/
n1 = 20;
console.log(n1);
...
- 비 엄격모드에서 변수에 대한 묵시적 선언을 해도 초기화를 하면 문제가 안되지만,
이와같이 묵시적 선언에 할당 (assignment) 연산을 하지 않은 경우에는 선언 자체가 인정되지 않음.
*/
// n2;
...
함수에 이와 같이 지역적으로 엄격모드 선언 가능. 단, 이 때에도 전역적으로 선언할 때와 마찬가지로
함수의 선두에 선언되어야만 적용.
*/
function test() {    
    // 'use strict';
...
/*
비 엄격모드에서의 묵시적 변수 선언은 함수 외부에서 바로 직접 참조는
안되지만, 함수 호출을 통해 전역변수처럼 참조 가능.
*/
// console.log(n2);
test();
console.log(n2);